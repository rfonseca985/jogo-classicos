...
import baseS.Elemento;
import baseS.Texto;
import baseS.Util;

/**
 * Implementação do cenário do jogo Snake com regras do jogo do russo.
 */
public class JogoCenarioDoRusso extends CenarioPadrao {
	...
	/**
	 * Construtor do cenário do jogo Snake com regras do jogo do russo.
	 *
	...
	public void carregar() {
		...
		serpente.setVel(JogoSnake.velocidade);
	...
	public void atualizar() {
		...
		if (!moveu) {
			if (dy != 0) {
				if (JogoSnake.controleTecla[JogoSnake.Tecla.ESQUERDA.ordinal()]) {
					dx = -1;

				} else if (JogoSnake.controleTecla[JogoSnake.Tecla.DIREITA.ordinal()]) {
					dx = 1;
				}
				...
				if (JogoSnake.controleTecla[JogoSnake.Tecla.CIMA.ordinal()]) {
					dy = -1;
				} else if (JogoSnake.controleTecla[JogoSnake.Tecla.BAIXO.ordinal()]) {
					dy = 1;
				}
	...
	/**
	 * Adiciona uma nova fruta no cenário, respeitando as condições de colisão com a serpente e os rastros.
	 *
	...
	public void desenhar(Graphics2D g) {
		...
		} else if (JogoSnake.pausado)
			JogoSnake.textoPausa.desenha(g, "PAUSA", largura / 2 - JogoSnake.textoPausa.getFonte().getSize(), altura / 2);
	}
