...
/**
 * Representa o cenário do jogo para um jogo semelhante ao Tetris.
 */
public class JogoCenario extends CenarioPadrao {

	/**
	 * Enumeração que representa diferentes estados do jogo.
	 */
	enum Estado {
	...
	private int largBloco, altBloco; // Largura e altura de um bloco
	private int ppx, ppy; // Posição da peça (x, y) na grade do jogo
	...
	private int nivel = JogoTetris.nivel;
	private int pontos;
	...
	/**
	 * Cria uma nova instância do cenário do jogo.
	 *
	...
	/**
	 * Carrega os recursos necessários para o cenário.
	 */
	@Override
	public void carregar() {
		...
		try {
			as = AudioSystem.getAudioInputStream(new File("Tetris/som/adiciona_peca.wav"));
			clipAdicionarPeca = AudioSystem.getClip();
			clipAdicionarPeca.open(as);

			as = AudioSystem.getAudioInputStream(new File("Tetris/som/109662_grunz_success.wav"));
			clipMarcarLinha = AudioSystem.getClip();
			clipMarcarLinha.open(as);

			seqSomDeFundo = MidiSystem.getSequencer();
			seqSomDeFundo.setSequence(MidiSystem.getSequence(new File("Tetris/som/piano_quebrado.mid")));
			seqSomDeFundo.open();
	...
	/**
	 * Libera os recursos utilizados pelo cenário.
	 */
	@Override
	...
	public void atualizar() {
		...
		if (JogoTetris.controleTecla[JogoTetris.Tecla.ESQUERDA.ordinal()]) {
			if (validaMovimento(peca, ppx - 1, ppy))
				ppx--;

		} else if (JogoTetris.controleTecla[JogoTetris.Tecla.DIREITA.ordinal()]) {
			if (validaMovimento(peca, ppx + 1, ppy))
				ppx++;
		}

		if (JogoTetris.controleTecla[JogoTetris.Tecla.CIMA.ordinal()]) {
			girarReposicionarPeca(false);

		} else if (JogoTetris.controleTecla[JogoTetris.Tecla.BAIXO.ordinal()]) {
			if (validaMovimento(peca, ppx, ppy + 1))
				ppy++;
		}

		if (depurar && JogoTetris.controleTecla[JogoTetris.Tecla.BC.ordinal()]) {
	...
	/**
	 * Adiciona uma nova peça à grade do jogo.
	 */
	...
	/**
	 * Adiciona a peça atual à grade do jogo.
	 */
	...
	/**
	 * Verifica se o movimento de uma peça é válido.
	 *
	 * @param peca A peça a ser movida.
	 * @param px   A posição x da peça.
	 * @param py   A posição y da peça.
	...
	/**
	 * Verifica se a peça parou fora da grade.
	 *
	 * @return true se a peça parou fora da grade, false caso contrário.
	 */
	...
	/**
	 * Verifica se houve colisão da peça com outra peça já existente na grade.
	 *
	 * @param px A posição x da peça.
	 * @param py A posição y da peça.
	...
	/**
	 * Marca as linhas completas na grade e realiza a animação.
	 *
	 * @return true se houve linha completa, false caso contrário.
	 */
	...
	/**
	 * Move as colunas após completar uma linha.
	 */
	 ...
	 *
	 * @param sentidoHorario true para girar no sentido horário, false para anti-horário.
	 */
	...
	/**
	 * Gira e reposiciona a peça no jogo.
	 *
	 * @param sentidoHorario true para girar no sentido horário, false para anti-horário.
	 */
